...
class Wx extends CI_Controller {
    ...
    */
    public function oauth_base($redirect_type = 0) {
        ...
        $token = $this->weixin->get_oauth_access_token($code);
        ...
        $url = $this->weixin->get_oauth_url(site_url('wx/oauth/' . $redirect_type), $state, true);
        //echo $url;
    ...
    * get user info 
    */
    public function oauth($redirect_type = 0) {
        $code = $this->input->get('code');
        $state = $this->input->get('state');
